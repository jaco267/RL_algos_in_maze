...
def qtransform_completed_by_mix_value(
    ...
    *,
    ...
    rescale_values: bool = True,  # if True, scale the qvalues by `1 / (max_q - min_q)`.
    use_mixed_value: bool = True, # if True, complete the Q-values with mixed value, otherwise complete the Q-values with the raw value.
    epsilon: chex.Numeric = 1e-8, # the minimum denominator when using `rescale_values`.
) -> chex.Array:
  ...
  #  0.1*[50+max(visit_counts)]*qvalues   #todo  replace this heuristic formula
  Args:
    tree: _unbatched_ MCTS tree state.
    node_index: scalar index of the parent node.
  ...
  """
  chex.assert_shape(node_index, ())
  qvalues = tree.qvalues(node_index)   #  shape(82,)
  visit_counts = tree.children_visits[node_index]   #(82,)
  # Computing the mixed value and producing completed_qvalues.
  raw_value = tree.raw_values[node_index]   #value shape = (,)
  prior_probs = jax.nn.softmax(             #prior shape = (82,)
      tree.children_prior_logits[node_index])
  if use_mixed_value:
    value = _compute_mixed_value(
        raw_value,       #*  Value get from NN
        qvalues=qvalues,
        visit_counts=visit_counts,
        prior_probs=prior_probs)
  else:
    value = raw_value
  completed_qvalues = _complete_qvalues(    #*  (82,)
      qvalues, visit_counts=visit_counts, value=value)   #it's  like what we do in duel Q network  #*   value=value from NN

  # Scaling the Q-values.
  if rescale_values:
    completed_qvalues = _rescale_qvalues(completed_qvalues, epsilon)
  maxvisit = jnp.max(visit_counts, axis=-1)
  # 0.1*[50+max(visit_counts)]*qvalues 
...
# done   #*  good idea  this can reduce the variance  if visit count == 0
def _complete_qvalues(qvalues, *, visit_counts, value):
  """Returns completed Q-values, with the `value` for unvisited actions."""
  chex.assert_equal_shape([qvalues, visit_counts])   # (82,)
  chex.assert_shape(value, [])                       #(,)
  # id_print(value.shape)   #*   ()
  # id_print(qvalues.shape)  #*  (5,)
  # The missing qvalues are replaced by the value.
  completed_qvalues = jnp.where(
      visit_counts > 0,
      qvalues,   
      value)    #mixed value  = 0.1*nn_value + 0.9*Σ π(a)q(a)  #better than nothing
...
def _compute_mixed_value(raw_value, qvalues, visit_counts, prior_probs):
  ...
  Appendix D.   Mixed Value approximation    # mixed the v_pi and  Σ π(a)q(a)
                                      # theoretically v_pi == Σ π(a)q(a), in optimal policy
  Args:
  ...
  """
  Appendix D.
    1/{1+ Σ N(b)} (  v_π  +       {Σ N(b)     /Σ(π(b))} * Σ π(a)q(a) )
   sum_visit_count raw_value  sum_visit_counts     weighted_q
                                              /sum_probs * prior_probs*qvalues                    
  """